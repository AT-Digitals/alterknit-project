...
export default function ShipInDetailsPage() {
  ...
  const filteredPeople = serviceData.filter(
    (item) =>
      item.more_details.latest_service !== "" &&
      item.more_details.previous_service !== ""
  );
  // console.log("fileter", filteredPeople);
  const finalArray = serviceData.filter(
    (item) => item.shipin_details.ShipInformation.firstName !== ""
  );
  ...
  const nextStep = () => {
    ...
    const existingItemIndex = serviceData.findIndex(
      (item) => item.services === serviceDetails.services
